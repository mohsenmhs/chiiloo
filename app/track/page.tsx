        ...
        <div className={styles.itemsList}>
          {order.items.map((item, index) => (
            <div key={index} className={styles.orderItem}>
              ...
              <div className={styles.itemQuantity}>{item.quantity} عدد</div>
...
function TrackOrderContent() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (searchType === "code") {
        query = query.eq("tracking_code", trackingCode.trim());
      } else {
      ...
      // If searching by tracking code, should only be one result
      if (searchType === "code") {
        setOrder(data[0] as Order);
      } else {
        // If searching by phone, can have multiple orders
        setOrders(data as Order[]);
        // If only one order, also set it for backward compatibility
        if (data.length === 1) {
          setOrder(data[0] as Order);
        }
      }
      ...
      <section className={styles.hero}>
        <div className="container">
          ...
          <p className={styles.heroSubtitle}>
            وضعیت سفارش خود را با کد پیگیری یا شماره تماس بررسی کنید
          </p>
        </div>
      </section>
